...
{
 while (*cp && ((*__ctype_b_loc ())[(int) ((*cp))] & (unsigned short int) _ISspace))
  cp++;
 return cp;
}
static char *skip_over_word(char *cp)
{
 ...
 while ((ch = *cp)) {
  ...
  if (((*__ctype_b_loc ())[(int) ((ch))] & (unsigned short int) _ISspace) || ch == '<' || ch == '>')
   break;
  cp++;
 }
 return cp;
...
static char *strip_line(char *line)
{
 ...
 while (*line) {
  if (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace))
   *p-- = '\0';
  else
   break;
 }
 ...
 *tmp = '\0';
 ...
 if (name == ((void *)0))
  return -12;
 ;
 if (!(*dev = blkid_get_dev(cache, name, 0x0001)))
  return -12;
 free(name);
 return 1;
}
...
static int parse_tag(blkid_cache cache, blkid_dev dev, char **cp)
{
 ...
 if (!strcmp(name, "DEVNO"))
  dev->bid_devno = strtoul(value, 0, 0);
...
void blkid_read_cache(blkid_cache cache)
{
 ...
 if (!cache)
  return;
 if ((fd = open(cache->bic_filename, 00)) < 0)
  return;
 if (fstat(fd, &st) < 0)
  goto errout;
 if ((st.st_mtim.tv_sec == cache->bic_ftime) ||
 ...
 fclose(file);
 cache->bic_flags &= ~0x0004;
 cache->bic_ftime = st.st_mtim.tv_sec;
 return;
errout:
 close(fd);
}
