...
static blkid_tag blkid_new_tag(void)
{
 ...
 do { (&tag->bit_tags)->next = (&tag->bit_tags); (&tag->bit_tags)->prev = (&tag->bit_tags); } while (0);
 do { (&tag->bit_names)->next = (&tag->bit_names); (&tag->bit_names)->prev = (&tag->bit_names); } while (0);
 return tag;
}
...
blkid_tag blkid_find_tag_dev(blkid_dev dev, const char *type)
{
 ...
 if (!dev || !type)
  return ((void *)0);
 for (p = (&dev->bid_tags)->next; p != (&dev->bid_tags); p = p->next) {
  blkid_tag tmp = ((struct blkid_struct_tag *)((char *)(p)-(unsigned long)(&((struct blkid_struct_tag *)0)->bit_tags)));
...
{
 blkid_tag head = ((void *)0), tmp;
 struct list_head *p;
 if (!cache || !type)
  return ((void *)0);
 for (p = (&cache->bic_tags)->next; p != (&cache->bic_tags); p = p->next) {
  tmp = ((struct blkid_struct_tag *)((char *)(p)-(unsigned long)(&((struct blkid_struct_tag *)0)->bit_tags)));
  if (!strcmp(tmp->bit_name, type)) {
   ;
   head = tmp;
   break;
    ...
    const char *value, const int vlength)
{
 ...
 if (!strcmp(name, "TYPE"))
  ...
  dev->bid_uuid = val;
 if (dev->bid_cache)
  dev->bid_cache->bic_flags |= 0x0004;
 return 0;
errout:
 blkid_free_tag(t);
 if (!t)
 ...
 *type = 0;
 *value = 0;
 if (!iter || iter->magic != 0x01a5284c ||
     iter->p == &iter->dev->bid_tags)
  return -1;
 tag = ((struct blkid_struct_tag *)((char *)(iter->p)-(unsigned long)(&((struct blkid_struct_tag *)0)->bit_tags)));
 *type = tag->bit_name;
 *value = tag->bit_val;
 iter->p = iter->p->next;
 return 0;
...
try_again:
 ...
 if (head) {
  for (p = (&head->bit_names)->next; p != (&head->bit_names); p = p->next) {
   blkid_tag tmp = ((struct blkid_struct_tag *)((char *)(p)-(unsigned long)(&((struct blkid_struct_tag *)0)->bit_names)));
 ...
 if (dev && !(dev->bid_flags & 0x0001)) {
  dev = blkid_verify(cache, dev);
  if (dev && (dev->bid_flags & 0x0001))
   goto try_again;
 }
 if (!dev && !(cache->bic_flags & 0x0002)) {
