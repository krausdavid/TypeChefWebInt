...
static int save_dev(blkid_dev dev, FILE *file)
{
 ...
 for (p = (&dev->bid_tags)->next; p != (&dev->bid_tags); p = p->next) {
  blkid_tag tag = ((struct blkid_struct_tag *)((char *)(p)-(unsigned long)(&((struct blkid_struct_tag *)0)->bit_tags)));
...
int blkid_flush_cache(blkid_cache cache)
{
 ...
 filename = cache->bic_filename ? cache->bic_filename: "/etc/blkid.tab";
 if (((ret = stat(filename, &st)) < 0 && errno != ENOENT) ||
     (ret == 0 && access(filename, 2) < 0)) {
  ;
  return 0;
 }
 ...
 for (p = (&cache->bic_devs)->next; p != (&cache->bic_devs); p = p->next) {
  blkid_dev dev = ((struct blkid_struct_dev *)((char *)(p)-(unsigned long)(&((struct blkid_struct_dev *)0)->bid_devs)));
  if (!dev->bid_type)
   continue;
