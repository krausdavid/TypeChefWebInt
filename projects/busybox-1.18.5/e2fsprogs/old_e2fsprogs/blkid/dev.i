...
blkid_dev blkid_new_dev(void)
{
 ...
 do { (&dev->bid_devs)->next = (&dev->bid_devs); (&dev->bid_devs)->prev = (&dev->bid_devs); } while (0);
 do { (&dev->bid_tags)->next = (&dev->bid_tags); (&dev->bid_tags)->prev = (&dev->bid_tags); } while (0);
 return dev;
}
void blkid_free_dev(blkid_dev dev)
{
 ...
 while (!list_empty(&dev->bid_tags)) {
  blkid_tag tag = ((struct blkid_struct_tag *)((char *)(dev->bid_tags.next)-(unsigned long)(&((struct blkid_struct_tag *)0)->bit_tags)));
  blkid_free_tag(tag);
 }
 if (dev->bid_name)
  free(dev->bid_name);
 ...
 *dev = 0;
 if (!iter || iter->magic != 0x01a5284c ||
  ...
  return -1;
 *dev = ((struct blkid_struct_dev *)((char *)(iter->p)-(unsigned long)(&((struct blkid_struct_dev *)0)->bid_devs)));
 iter->p = iter->p->next;
 return 0;
}
