...
int blkid_get_cache(blkid_cache *ret_cache, const char *filename)
{
 ...
 do { (&cache->bic_devs)->next = (&cache->bic_devs); (&cache->bic_devs)->prev = (&cache->bic_devs); } while (0);
 do { (&cache->bic_tags)->next = (&cache->bic_tags); (&cache->bic_tags)->prev = (&cache->bic_tags); } while (0);
 if (filename && !strlen(filename))
  filename = 0;
 if (!filename && (getuid() == geteuid()))
  filename = getenv("BLKID_FILE");
 if (!filename)
  filename = "/etc/blkid.tab";
...
void blkid_put_cache(blkid_cache cache)
{
 ...
 while (!list_empty(&cache->bic_devs)) {
  blkid_dev dev = ((struct blkid_struct_dev *)((char *)(cache->bic_devs.next)-(unsigned long)(&((struct blkid_struct_dev *)0)->bid_devs)));
  blkid_free_dev(dev);
 }
 while (!list_empty(&cache->bic_tags)) {
  blkid_tag tag = ((struct blkid_struct_tag *)((char *)(cache->bic_tags.next)-(unsigned long)(&((struct blkid_struct_tag *)0)->bit_tags)));
  while (!list_empty(&tag->bit_names)) {
   blkid_tag bad = ((struct blkid_struct_tag *)((char *)(tag->bit_names.next)-(unsigned long)(&((struct blkid_struct_tag *)0)->bit_names)));
   ;
   blkid_free_tag(bad);
  }
  blkid_free_tag(tag);
