...
errcode_t ext2fs_create_resize_inode(ext2_filsys fs)
{
 ...
 if (retval)
  goto out_free;
 apb = (((1 << 10) << (sb)->s_log_block_size) / sizeof(__u32));
 rsv_add = fs->blocksize / 512;
 if ((dindir_blk = inode.i_block[(12 + 1)])) {
  ...
  if (retval)
   goto out_free;
  inode.i_mode = 0100000 | 0600;
  inode.i_links_count = 1;
  inode.i_block[(12 + 1)] = dindir_blk;
  inode.i_blocks = rsv_add;
  memset(dindir_buf, 0, fs->blocksize);
  dindir_dirty = inode_dirty = 1;
  inode_size = apb*apb + apb + 12;
  inode_size *= fs->blocksize;
  inode.i_size = inode_size & 0xFFFFFFFF;
  inode.i_dir_acl = (inode_size >> 32) & 0xFFFFFFFF;
  if (inode.i_dir_acl) {
...
out_inode:
 if (inode_dirty) {
  inode.i_atime = inode.i_mtime = time(((void *)0));
  retval2 = ext2fs_write_inode(fs, 7, &inode);
  if (!retval)
   retval = retval2;
 }
out_free:
