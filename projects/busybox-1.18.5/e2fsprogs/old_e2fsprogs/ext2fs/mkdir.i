         ...
         const char *name)
{
 ...
 if (!ino) {
  retval = ext2fs_new_inode(fs, parent, 0040000 | 0755,
       0, &ino);
  if (retval)
   goto cleanup;
 }
 ...
 inode.i_mode = 0040000 | (0777 & ~fs->umask);
 inode.i_uid = inode.i_gid = 0;
 inode.i_blocks = fs->blocksize / 512;
 inode.i_block[0] = blk;
 inode.i_links_count = 2;
 inode.i_ctime = inode.i_atime = inode.i_mtime = time(((void *)0));
 ...
 if (name) {
  ...
  if (retval != (2133571404L))
   goto cleanup;
  retval = ext2fs_link(fs, parent, name, ino, 2);
  if (retval)
   goto cleanup;
 }
 if (parent != ino) {
