    ...
    int ref_offset, void *priv_data)
{
 ...
 if (ext2fs_test_block_bitmap(pb->reserve, block)) {
  do {
   ...
   if (block == orig) {
    pb->error = (2133571400L);
    return 2;
   }
  } while (ext2fs_test_block_bitmap(pb->reserve, block) ||
    ext2fs_test_block_bitmap(pb->alloc_map, block));
  retval = ((fs->io)->manager->read_blk((fs->io),orig,1,pb->buf));
  if (retval) {
   pb->error = retval;
   return 2;
  }
  retval = ((fs->io)->manager->write_blk((fs->io),block,1,pb->buf));
  if (retval) {
   pb->error = retval;
   return 2;
  }
        ...
        int flags)
{
 ...
 while (ino) {
  ...
  pb.ino = ino;
  pb.inode = &inode;
  pb.add_dir = ((((inode.i_mode) & 00170000) == 0040000) &&
