...
blk_t ext2fs_descriptor_block_loc(ext2_filsys fs, blk_t group_block, dgrp_t i)
{
 ...
 int has_super = 0;
 int ret_blk;
 if (!(fs->super->s_feature_incompat & 0x0010) ||
         ...
         ext2_filsys *ret_fs)
{
 ...
 char *dest, *cp;
 if ((manager)->magic != ((2133571335L))) return ((2133571335L));
 ...
 fs->image_io = fs->io;
 fs->io->app_data = fs;
 retval = ext2fs_get_mem(1024, &fs->super);
 if (retval)
  goto cleanup;
 if (flags & 0x2000) {
  ...
  if (retval)
   goto cleanup;
  retval = ((fs->io)->manager->read_blk((fs->io),0,-(int)sizeof(struct ext2_image_hdr),fs->image_header));
  if (retval)
   goto cleanup;
  if (fs->image_header->magic_number != (2133571344L))
   return (2133571344L);
  superblock = 1;
 ...
 if (superblock) {
  ...
  ((fs->io)->manager->set_blksize((fs->io),block_size));
  group_block = superblock;
  fs->orig_super = 0;
 } else {
  ((fs->io)->manager->set_blksize((fs->io),1024));
  superblock = 1;
  group_block = 0;
  retval = ext2fs_get_mem(1024, &fs->orig_super);
  if (retval)
   goto cleanup;
 }
 retval = ((fs->io)->manager->read_blk((fs->io),superblock,-1024,fs->super));
 if (retval)
  goto cleanup;
 if (fs->orig_super)
  memcpy(fs->orig_super, fs->super, 1024);
 if (fs->super->s_magic != 0xEF53) {
  retval = (2133571347L);
  goto cleanup;
 }
 if (fs->super->s_rev_level > 1) {
  retval = (2133571348L);
  goto cleanup;
 }
 if (!(flags & 0x400)) {
  if (fs->super->s_feature_incompat &
      ~(0x0002| 0x0008| 0x0010| 0x0004)) {
   retval = (2133571395L);
   goto cleanup;
  }
  if ((flags & 0x01) &&
 ...
 fs->blocksize = ((1 << 10) << (fs->super)->s_log_block_size);
 if (fs->blocksize == 0) {
  retval = (2133571388L);
  goto cleanup;
 }
 fs->fragsize = ((1 << 10) << (fs->super)->s_log_frag_size);
 fs->inode_blocks_per_group = ((fs->super->s_inodes_per_group *
           (((fs->super)->s_rev_level == 0) ? 128 : (fs->super)->s_inode_size) +
           ((1 << 10) << (fs->super)->s_log_block_size) - 1) /
          ((1 << 10) << (fs->super)->s_log_block_size));
 if (block_size) {
 ...
 if (blocks_per_group == 0 ||
     blocks_per_group > ((1 << 16) - 8) ||
     fs->inode_blocks_per_group > ((1 << 16) - (((1 << 10) << (fs->super)->s_log_block_size)/(((fs->super)->s_rev_level == 0) ? 128 : (fs->super)->s_inode_size)))) {
  retval = (2133571388L);
  goto cleanup;
 }
 ...
 fs->desc_blocks = (fs->group_desc_count +
      (((1 << 10) << (fs->super)->s_log_block_size) / sizeof (struct ext2_group_desc)) - 1)
  / (((1 << 10) << (fs->super)->s_log_block_size) / sizeof (struct ext2_group_desc));
 ...
 for (i = 0; i < fs->desc_blocks; i++) {
  ...
  retval = ((fs->io)->manager->read_blk((fs->io),blk,1,dest));
  if (retval)
   goto cleanup;
  dest += fs->blocksize;
 }
