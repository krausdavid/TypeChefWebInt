...
{
 if (!fs || (fs->magic != (2133571329L)))
  return;
 if (fs->image_io != fs->io) {
  if (fs->image_io)
   ((fs->image_io)->manager->close((fs->image_io)));
 }
 if (fs->io) {
...
{
 if (!bitmap || (bitmap->magic != (2133571338L)))
  return;
 bitmap->magic = 0;
...
{
 if (!dblist || (dblist->magic != (2133571340L)))
  return;
