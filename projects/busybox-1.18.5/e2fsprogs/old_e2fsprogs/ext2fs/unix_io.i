         ...
         int count, void *buf)
{
 ...
 if (lseek(data->dev, location, 0) != location) {
  retval = errno ? errno : (2133571397L);
  goto error_out;
 }
        ...
        struct unix_private_data *data)
{
 ...
 int i;
 data->access_time = 0;
 for (i=0, cache = data->cache; i < 8; i++, cache++) {
  cache->block = 0;
  cache->access_time = 0;
  cache->dirty = 0;
  cache->in_use = 0;
...
static errcode_t unix_open(const char *name, int flags, io_channel *channel)
{
 ...
 struct unix_private_data *data = ((void *)0);
 errcode_t retval;
 int open_flags;
 struct stat st;
 struct utsname ut;
 ...
 data->magic = (2133571334L);
 if ((retval = alloc_cache(io, data)))
  goto cleanup;
 open_flags = (flags & 1) ? 02 : 00;
...
static errcode_t unix_close(io_channel channel)
{
 ...
 if ((data)->magic != ((2133571334L))) return ((2133571334L));
...
{
 struct unix_private_data *data;
 struct unix_cache *cache, *reuse[4];
 errcode_t retval;
 char *cp;
 int i, j;
 if ((channel)->magic != ((2133571333L))) return ((2133571333L));
    ...
    int count, const void *buf)
{
 ...
 writethrough = channel->flags & 0x01;
 if (writethrough)
     ...
     int size, const void *buf)
{
 ...
 if ((retval = flush_cached_blocks(channel, data, 1)))
  return retval;
 if (lseek(data->dev, offset + data->offset, 0) < 0)
  return errno;
 actual = write(data->dev, buf, size);
 if (actual != size)
  return (2133571365L);
