...
errcode_t ext2fs_write_inode_bitmap(ext2_filsys fs)
{
 ...
 if (!inode_bitmap)
  return 0;
 nbytes = (size_t) ((((fs->super)->s_inodes_per_group)+7) / 8);
 ...
 for (i = 0; i < fs->group_desc_count; i++) {
  ...
  if (blk) {
   retval = ((fs->io)->manager->write_blk((fs->io),blk,1,bitmap_block));
   if (retval)
    return (2133571355L);
  }
  inode_bitmap += nbytes;
...
errcode_t ext2fs_write_block_bitmap (ext2_filsys fs)
{
 ...
 if (!block_bitmap)
  return 0;
 nbytes = ((fs->super)->s_blocks_per_group) / 8;
 retval = ext2fs_get_mem(fs->blocksize, &bitmap_block);
 if (retval)
  return retval;
 memset(bitmap_block, 0xff, fs->blocksize);
...
{
 dgrp_t i;
 char *block_bitmap = ((void *)0), *inode_bitmap = ((void *)0);
 char *buf;
 errcode_t retval;
 int block_nbytes = (int) ((fs->super)->s_blocks_per_group) / 8;
 int inode_nbytes = (int) ((fs->super)->s_inodes_per_group) / 8;
 blk_t blk;
 if ((fs)->magic != ((2133571329L))) return ((2133571329L));
 ...
 if (fs->flags & 0x2000) {
  if (inode_bitmap) {
   blk = (fs->image_header->offset_inodemap /
          fs->blocksize);
   retval = ((fs->image_io)->manager->read_blk((fs->image_io),blk,-(inode_nbytes * fs->group_desc_count),inode_bitmap));
   if (retval)
    goto cleanup;
  }
  if (block_bitmap) {
   blk = (fs->image_header->offset_blockmap /
          fs->blocksize);
   retval = ((fs->image_io)->manager->read_blk((fs->image_io),blk,-(block_nbytes * fs->group_desc_count),block_bitmap));
   if (retval)
    goto cleanup;
  }
  return 0;
 ...
 for (i = 0; i < fs->group_desc_count; i++) {
  if (block_bitmap) {
   blk = fs->group_desc[i].bg_block_bitmap;
   if (blk) {
    retval = ((fs->io)->manager->read_blk((fs->io),blk,-block_nbytes,block_bitmap));
    if (retval) {
     retval = (2133571358L);
     goto cleanup;
    }
  ...
  if (inode_bitmap) {
   blk = fs->group_desc[i].bg_inode_bitmap;
   if (blk) {
    retval = ((fs->io)->manager->read_blk((fs->io),blk,-inode_nbytes,inode_bitmap));
    if (retval) {
     retval = (2133571356L);
     goto cleanup;
    }
