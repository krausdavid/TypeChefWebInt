    ...
    void *priv_data)
{
 ...
 if (*(blocks + 12) || (flags & 1)) {
  ret |= block_iterate_ind(blocks + 12,
      0, 12, &ctx);
  if (ret & 2)
   goto abort_exit;
 } else
  ctx.bcount += limit;
 if (*(blocks + (12 + 1)) || (flags & 1)) {
  ret |= block_iterate_dind(blocks + (12 + 1),
       0, (12 + 1), &ctx);
  if (ret & 2)
   goto abort_exit;
 } else
  ctx.bcount += limit * limit;
 if (*(blocks + ((12 + 1) + 1)) || (flags & 1)) {
  ret |= block_iterate_tind(blocks + ((12 + 1) + 1),
       0, ((12 + 1) + 1), &ctx);
  if (ret & 2)
   goto abort_exit;
 }
          ...
          void *priv_data)
{
 ...
 xl.func = func;
 return ext2fs_block_iterate2(fs, ino, 0x1000 | flags,
