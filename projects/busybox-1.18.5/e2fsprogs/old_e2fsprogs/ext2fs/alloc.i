      ...
      ext2fs_inode_bitmap map, ext2_ino_t *ret)
{
 ...
 if (!map)
  return (2133571367L);
 if (dir > 0)
  dir_group = (dir - 1) / ((fs->super)->s_inodes_per_group);
 start_inode = (dir_group * ((fs->super)->s_inodes_per_group)) + 1;
 if (start_inode < (((fs->super)->s_rev_level == 0) ? 11 : (fs->super)->s_first_ino))
  start_inode = (((fs->super)->s_rev_level == 0) ? 11 : (fs->super)->s_first_ino);
 i = start_inode;
 do {
        ...
        char *block_buf, blk_t *ret)
{
 ...
 if (retval)
  goto fail;
 retval = ((fs->io)->manager->write_blk((fs->io),block,1,block_buf));
 if (retval)
  goto fail;
