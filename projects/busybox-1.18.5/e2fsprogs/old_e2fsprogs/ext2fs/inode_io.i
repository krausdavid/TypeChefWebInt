...
errcode_t ext2fs_inode_io_intern(ext2_filsys fs, ext2_ino_t ino,
     char **name)
{
 return ext2fs_inode_io_intern2(fs, ino, ((void *)0), name);
}
static errcode_t inode_open(const char *name, int flags, io_channel *channel)
{
 io_channel io = ((void *)0);
 struct inode_private_data *prev, *data = ((void *)0);
 errcode_t retval;
 int open_flags;
 if (name == 0)
  return (2133571386L);
 for (data = top_intern, prev = ((void *)0); data;
...
static errcode_t inode_close(io_channel channel)
{
 ...
 if ((data)->magic != ((2133571345L))) return ((2133571345L));
