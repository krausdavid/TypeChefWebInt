       ...
       int flags, ext2_file_t *ret)
{
 ...
 file->magic = (2133571343L);
 file->fs = fs;
 file->ino = ino;
 file->flags = flags & 0x00FF;
 if (inode) {
...
errcode_t ext2fs_file_flush(ext2_file_t file)
{
 errcode_t retval;
 ext2_filsys fs;
 if ((file)->magic != ((2133571343L))) return ((2133571343L));
 fs = file->fs;
 ...
 if (!file->physblock) {
  retval = ext2fs_bmap(fs, file->ino, &file->inode,
         (file->buf + fs->blocksize), file->ino ? 0x0001 : 0,
         file->blockno, &file->physblock);
  if (retval)
   return retval;
 }
 retval = ((fs->io)->manager->write_blk((fs->io),file->physblock,1,file->buf));
 if (retval)
  return retval;
 file->flags &= ~0x4000;
 return retval;
...
static errcode_t load_buffer(ext2_file_t file, int dontfill)
{
 ...
 if (!(file->flags & 0x2000)) {
  retval = ext2fs_bmap(fs, file->ino, &file->inode,
         (file->buf + fs->blocksize), 0, file->blockno,
         &file->physblock);
  if (retval)
   return retval;
  if (!dontfill) {
   if (file->physblock) {
    retval = ((fs->io)->manager->read_blk((fs->io),file->physblock,1,file->buf));
    if (retval)
     return retval;
   } else
      ...
      unsigned int wanted, unsigned int *got)
{
 ...
 if ((file)->magic != ((2133571343L))) return ((2133571343L));
 fs = file->fs;
 while ((file->pos < ((&file->inode)->i_size | ((__u64) (&file->inode)->i_dir_acl << 32))) && (wanted > 0)) {
  ...
  if (c > wanted)
   c = wanted;
  left = ((&file->inode)->i_size | ((__u64) (&file->inode)->i_dir_acl << 32)) - file->pos;
  if (c > left)
   c = left;
       ...
       int whence, __u64 *ret_pos)
{
 ...
 if (whence == 0)
  ...
  file->pos = ((&file->inode)->i_size | ((__u64) (&file->inode)->i_dir_acl << 32)) + offset;
 else
  return (2133571399L);
 if (ret_pos)
  *ret_pos = file->pos;
...
errcode_t ext2fs_file_get_lsize(ext2_file_t file, __u64 *ret_size)
{
 if (file->magic != (2133571343L))
  return (2133571343L);
 *ret_size = ((&file->inode)->i_size | ((__u64) (&file->inode)->i_dir_acl << 32));
 return 0;
}
