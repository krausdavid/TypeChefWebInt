       ...
       struct ext2_super_block *super)
{
 ...
 if (!fs->io->manager->write_byte || !fs->orig_super) {
  ...
  retval = ((fs->io)->manager->write_blk((fs->io),1,-1024,super));
 ...
 for (check_idx = 0; check_idx < 1024/2; check_idx++) {
  ...
  retval = io_channel_write_byte(fs->io,
          1024 + (2 * write_idx), size,
            new_super + write_idx);
  if (retval)
   return retval;
 }
 memcpy(fs->orig_super, super, 1024);
 return 0;
}
        ...
        struct ext2_super_block *super_shadow)
{
 ...
 return ((fs->io)->manager->write_blk((fs->io),group_block,-1024,super_shadow));
}
errcode_t ext2fs_flush(ext2_filsys fs)
{
 ...
 struct ext2_super_block *super_shadow = ((void *)0);
 struct ext2_group_desc *group_shadow = ((void *)0);
 char *group_ptr;
 int old_desc_blocks;
 if ((fs)->magic != ((2133571329L))) return ((2133571329L));
 fs_state = fs->super->s_state;
 fs->super->s_wtime = time(((void *)0));
 ...
 for (i = 0; i < fs->group_desc_count; i++) {
  ...
  if ((old_desc_blk) &&
      (!(fs->flags & 0x200) || (i == 0))) {
   retval = ((fs->io)->manager->write_blk((fs->io),old_desc_blk,old_desc_blocks,group_ptr));
   if (retval)
    goto errout;
  }
  if (new_desc_blk) {
   retval = ((fs->io)->manager->write_blk((fs->io),new_desc_blk,1,group_ptr + (meta_bg*fs->blocksize)));
   if (retval)
    goto errout;
  }
 }
...
write_primary_superblock_only:
 retval = ((fs->io)->manager->flush((fs->io)));
