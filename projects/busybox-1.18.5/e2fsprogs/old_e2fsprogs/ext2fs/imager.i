...
errcode_t ext2fs_image_inode_write(ext2_filsys fs, int fd, int flags)
{
 ...
 for (group = 0; group < fs->group_desc_count; group++) {
  ...
  while (left) {
   ...
   if (c > left)
    c = left;
   retval = ((fs->io)->manager->read_blk((fs->io),blk,c,buf));
   if (retval)
    goto errout;
   cp = buf;
   while (c) {
      ...
      int flags )
{
 ...
 for (group = 0; group < fs->group_desc_count; group++) {
  ...
  while (left) {
   ...
   retval = ((fs->io)->manager->write_blk((fs->io),blk,c,buf));
   if (retval)
    goto errout;
   blk += c;
   left -= c;
...
errcode_t ext2fs_image_bitmap_write(ext2_filsys fs, int fd, int flags)
{
 ...
 if (flags & 1) {
  ...
  size = (((fs->super)->s_inodes_per_group) / 8);
 } else {
  if (!fs->block_map) {
