     ...
     ext2_inode_scan *ret_scan)
{
 ...
 if ((fs)->magic != ((2133571329L))) return ((2133571329L));
 if (fs->badblocks == 0) {
 ...
 scan->magic = (2133571332L);
 scan->fs = fs;
 scan->inode_size = (((fs->super)->s_rev_level == 0) ? 128 : (fs->super)->s_inode_size);
 scan->bytes_left = 0;
 ...
 scan->inodes_left = ((scan->fs->super)->s_inodes_per_group);
 ...
 if (scan->fs->badblocks && scan->fs->badblocks->num)
  scan->scan_flags |= 0x0001;
 *ret_scan = scan;
 return 0;
}
void ext2fs_close_inode_scan(ext2_inode_scan scan)
{
 if (!scan || (scan->magic != (2133571332L)))
  return;
 ext2fs_free_mem(&scan->inode_buffer);
...
static errcode_t get_next_blockgroup(ext2_inode_scan scan)
{
 ...
 scan->current_inode = scan->current_group *
  ((scan->fs->super)->s_inodes_per_group);
 scan->bytes_left = 0;
 scan->inodes_left = ((scan->fs->super)->s_inodes_per_group);
 scan->blocks_left = scan->fs->inode_blocks_per_group;
 return 0;
...
static errcode_t get_next_blocks(ext2_inode_scan scan)
{
 ...
 if (scan->scan_flags & 0x0002) {
  if (scan->bytes_left)
   scan->scan_flags |= 0x0004;
  scan->scan_flags &= ~0x0002;
 }
 if (scan->scan_flags & 0x0001) {
 ...
 if ((scan->scan_flags & 0x0002) ||
  ...
  retval = ((scan->fs->io)->manager->read_blk((scan->fs->io),scan->current_block,(int) num_blocks,scan->inode_buffer));
  if (retval)
   return (2133571361L);
 }
         ...
         struct ext2_inode *inode, int bufsize)
{
 errcode_t retval;
 int extra_bytes = 0;
 if ((scan)->magic != ((2133571332L))) return ((2133571332L));
     ...
     struct ext2_inode * inode, int bufsize)
{
 ...
 if ((ino == 0) || (ino > fs->super->s_inodes_count))
  return (2133571369L);
 if (fs->flags & 0x2000) {
  inodes_per_block = fs->blocksize / (((fs->super)->s_rev_level == 0) ? 128 : (fs->super)->s_inode_size);
  ...
  offset = ((ino - 1) % ((fs->super)->s_inodes_per_group)) *
   (((fs->super)->s_rev_level == 0) ? 128 : (fs->super)->s_inode_size);
  block = offset >> ((fs->super)->s_log_block_size + 10);
 ...
 offset &= (((1 << 10) << (fs->super)->s_log_block_size) - 1);
 length = (((fs->super)->s_rev_level == 0) ? 128 : (fs->super)->s_inode_size);
 if (bufsize < length)
  length = bufsize;
 ptr = (char *) inode;
 while (length) {
  ...
  if (block_nr != fs->icache->buffer_blk) {
   retval = ((io)->manager->read_blk((io),block_nr,1,fs->icache->buffer));
   if (retval)
    return retval;
      ...
      struct ext2_inode * inode, int bufsize)
{
 ...
 if ((ino == 0) || (ino > fs->super->s_inodes_count))
  return (2133571369L);
 length = bufsize;
 if (length < (((fs->super)->s_rev_level == 0) ? 128 : (fs->super)->s_inode_size))
  length = (((fs->super)->s_rev_level == 0) ? 128 : (fs->super)->s_inode_size);
 ...
 while (length) {
  ...
  if (fs->icache->buffer_blk != block_nr) {
   retval = ((fs->io)->manager->read_blk((fs->io),block_nr,1,fs->icache->buffer));
   if (retval)
    goto errout;
   fs->icache->buffer_blk = block_nr;
  }
  memcpy((char *) fs->icache->buffer + (unsigned) offset,
         ptr, clen);
  retval = ((fs->io)->manager->write_blk((fs->io),block_nr,1,fs->icache->buffer));
  if (retval)
   goto errout;
  offset = 0;
  ptr += clen;
     ...
     struct ext2_inode *inode)
{
 ...
 int size = (((fs->super)->s_rev_level == 0) ? 128 : (fs->super)->s_inode_size);
