...
errcode_t ext2fs_update_bb_inode(ext2_filsys fs, ext2_badblocks_list bb_list)
{
 ...
 rec.err = 0;
 retval = ext2fs_block_iterate2(fs, 1,
           2, 0,
 ...
 retval = ext2fs_read_inode(fs, 1, &inode);
 if (retval)
  goto cleanup;
 ...
 retval = ext2fs_write_inode(fs, 1, &inode);
 if (retval)
  goto cleanup;
cleanup:
         ...
         void *priv_data)
{
 ...
 retry:
  ...
  retval = ((fs->io)->manager->write_blk((fs->io),blk,1,rec->block_buf));
  if (retval) {
   rec->err = retval;
   return 2;
  }
