...
void exec_kernel_doc(char **svec)
{
 ...
 switch(pid=fork()) {
  ...
  case 0:
   ...
   rflen += strlen("scripts/" "kernel-doc");
   real_filename = __builtin_alloca (rflen + 1);
   strcpy(real_filename, getenv("SRCTREE"));
   strcat(real_filename, "scripts/" "kernel-doc");
 ...
 if (((((__extension__ (((union { __typeof(ret) __in; int __i; }) { .__in = (ret) }).__i))) & 0x7f) == 0))
  exitstatus |= ((((__extension__ (((union { __typeof(ret) __in; int __i; }) { .__in = (ret) }).__i))) & 0xff00) >> 8);
 else
  exitstatus = 0xff;
}
struct symbols
...
void find_export_symbols(char * filename)
{
 ...
 if (filename_exist(filename) == ((void *)0)) {
  ...
  char *real_filename = __builtin_alloca (rflen + 1);
  strcpy(real_filename, getenv("SRCTREE"));
  ...
  while (fgets(line, 2048, fp)) {
   ...
   if (((p = strstr(line, "EXPORT_SYMBOL_GPL")) != 0) ||
    ...
    while (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISalnum) || *p == '_')
     p++;
    ...
    while (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace))
     p++;
    e = p;
    while (((*__ctype_b_loc ())[(int) ((*e))] & (unsigned short int) _ISalnum) || *e == '_')
     e++;
    *e = '\0';
...
void singfunc(char * filename, char * line)
{
 ...
 vec[idx++] = "-docbook";
 for (i=0; line[i]; i++) {
  if (((*__ctype_b_loc ())[(int) ((line[i]))] & (unsigned short int) _ISspace)) {
   line[i] = '\0';
   startofsym = 1;
   continue;
  }
...
void parse_file(FILE *infile)
{
 ...
 while (fgets(line, 2048, infile)) {
  if (line[0] == '!') {
   ...
   switch (line[1]) {
    case 'E':
     while (*s && !((*__ctype_b_loc ())[(int) ((*s))] & (unsigned short int) _ISspace)) s++;
     *s = '\0';
