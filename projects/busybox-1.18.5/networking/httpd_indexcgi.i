...
static void fmt_str( const char *src)
{
...
static void fmt_url( const char *name)
{
 while (*name) {
  unsigned c = *name++;
  guarantee(3);
...
static void fmt_html( const char *name)
{
 while (*name) {
  char c = *name++;
  if (c == '<')
...
static void fmt_ull( unsigned long long n)
{
...
static void fmt_02u( unsigned n)
{
 dst[0] = (n / 10) + '0';
 dst[1] = (n % 10) + '0';
 dst += 2;
}
static void fmt_04u( unsigned n)
{
 fmt_02u(n / 100);
 fmt_02u(n % 100);
}
int main(int argc, char *argv[])
{
 ...
 while (1) {
  ...
  dir_list[dir_list_count].dl_mtime = sb.st_mtim.tv_sec;
  dir_list_count++;
 }
 closedir(dirp);
 ...
 while (dir_list_count--) {
  struct tm *ptm;
  if (((((cdir->dl_mode)) & 0170000) == (0040000))) {
   count_dirs++;
  } else if (((((cdir->dl_mode)) & 0170000) == (0100000))) {
   count_files++;
  ...
  *dst++ = (odd ? 'o' : 'e');
  ...
  fmt_url(cdir->dl_name);
  if (((((cdir->dl_mode)) & 0170000) == (0040000)))
   *dst++ = '/';
  fmt_str("'>");
  fmt_html(cdir->dl_name);
  if (((((cdir->dl_mode)) & 0170000) == (0040000)))
   *dst++ = '/';
  fmt_str("</a><td class=sz>");
  if (((((cdir->dl_mode)) & 0170000) == (0100000)))
